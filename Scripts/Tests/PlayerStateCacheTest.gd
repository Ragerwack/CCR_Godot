...
func _ready() -> void:
	...
	if str(empty_request.get("relic_locale", "")) != "zh-CN":
		return _fail("new_device_locale_missing")
	...
	var relics: Array = [{
		...
		"combat_power": 20,
		"status": "active",
		"deck_def": {"id": 21, "name": "solar_system__sun", "description": "太阳"},
		"series": {"id": 2, "name": "太阳系"},
	}]
	if not cache.save_snapshot(TEST_PLAYER_ID, 7, 12, "zh-CN", identity, relics):
	...
	file.store_string("{\"cache_schema_version\":1,\"player_id\":987654321,\"payload_json\":\"tampered\",\"payload_sha256\":\"bad\"}")
	file.close()
	if not cache.load_snapshot(TEST_PLAYER_ID).is_empty():
		return _fail("corrupt_checksum_was_accepted")

	cache.remove_snapshot(TEST_PLAYER_ID)
	print("PLAYER_STATE_CACHE ok new_device=true hit=true corruption_rejected=true")
	get_tree().quit(0)
